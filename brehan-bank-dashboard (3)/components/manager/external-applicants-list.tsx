...
interface ManagerExternalApplicantsListProps {
  status: string
  searchTerm: string
  filterJob: string
}

export function ManagerExternalApplicantsList({ status, searchTerm, filterJob }: ManagerExternalApplicantsListProps) {
    ...
    const fetchApplications = async () => {
      try {
        ...
        if (filterJob && filterJob !== "all") {
          response = await jobAPI.getExternalApplicationsByJob(filterJob)
        } else {
          response = await jobAPI.getAllExternalApplications()
        }
        ...
        if (searchTerm) {
          ...
          filteredApps = filteredApps.filter(
            (app) =>
              ...
              app.field_of_study?.toLowerCase().includes(term),
          )
        }
