...
import { PlusCircle, Search, SortDesc, Filter } from "lucide-react"
...
export function AdminEmployeesPage() {
  ...
  const handleCreateEmployee = async (employeeData: EmployeeData) => {
    try {
  ...
  const handleUpdateEmployee = async (employeeData: EmployeeData) => {
    if (!selectedEmployee?.id) return

    try {
      await employeeAPI.updateEmployee(selectedEmployee.id.toString(), employeeData)
      toast({
        title: "Success",
  ...
  const handleDeleteEmployee = async () => {
    ...
    try {
      await employeeAPI.deleteEmployee(selectedEmployee.id.toString())
      toast({
        title: "Success",
    ...
    <div className="container mx-auto py-6">
      ...
      <Tabs defaultValue="all" value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="mb-4">
          ...
          <TabsTrigger value="highScore">High Scores (70+)</TabsTrigger>
          <TabsTrigger value="mediumScore">Medium Scores (50-70)</TabsTrigger>
          <TabsTrigger value="lowScore">Low Scores (&lt;50)</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="highScore">
          <Card>
            <CardHeader>
              <CardTitle>High Scoring Employees (70+)</CardTitle>
            </CardHeader>
            <CardContent>
              <EmployeesList
        ...
        <TabsContent value="mediumScore">
          <Card>
            <CardHeader>
              <CardTitle>Medium Scoring Employees (50-70)</CardTitle>
            </CardHeader>
            <CardContent>
              <EmployeesList
                employees={filteredEmployees.filter((e) => (e.total || 0) >= 50 && (e.total || 0) < 70)}
                isLoading={isLoading}
                onEdit={openEditDialog}
                onDelete={openDeleteDialog}
                onView={openViewDialog}
        ...
        <TabsContent value="lowScore">
          <Card>
            <CardHeader>
              <CardTitle>Low Scoring Employees (&lt;50)</CardTitle>
            </CardHeader>
            <CardContent>
              <EmployeesList
                employees={filteredEmployees.filter((e) => (e.total || 0) < 50)}
                isLoading={isLoading}
                onEdit={openEditDialog}
                onDelete={openDeleteDialog}
                onView={openViewDialog}
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Fill in the employee details below to create a new employee record.</DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          {selectedEmployee && <EmployeeForm employee={selectedEmployee} onSubmit={handleUpdateEmployee} />}
        </DialogContent>
      </Dialog>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <p>
                    <span className="font-medium">TMD Rec (20%):</span> {selectedEmployee.tmdrec20}
                  </p>
                  <p>
                    <span className="font-medium">District Rec (15%):</span> {selectedEmployee.disrec15}
                  </p>
                  <p>
                    <span className="font-medium">Total Score:</span>
                    <Badge variant={selectedEmployee.total > 70 ? "secondary" : "default"}>{selectedEmployee.total}</Badge>
                  </p>
                </div>
              </div>
            </div>
